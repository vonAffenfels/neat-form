...
module.exports = class Field {
    ...
    /**
     * Checks if the default value is set in the config and it is
     * a promise to be resolved.
     *
     * @returns {*|{}|boolean}
     */
    hasLazyDefault() {
        return this.config && this.config.default && typeof this.config.default.then == 'function';
    }
    ...
    getValue() {
        ...
        if (form && form.isUpdate() && !form.submitted && !form.validating) {
            if (this.config.onUpdate && this.config.onUpdate.clear) {
                this.setValue(this.config.default || null, true);
            }
        }

        if (!this.valueSet && this.config.default) {
            if (this.hasLazyDefault()) {
                return this.config.default.then((val) => {
    ...
    validate() {
        ...
        return Promise.map(this.validators, (validatorConfig) => {
            ...
            if ((this.errors instanceof Object) && Object.keys(this.errors).length === 0) {
                this.errors = null;
            }
